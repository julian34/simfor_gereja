...
class Mksp extends CI_Model
{
    ...
    public function buat_table($data)
    {
        ...
        foreach ($data as $row)
        {
            $this->table->add_row(
                ...
                .
                anchor('backend-sekolah-hapus/'.$row->id_ksp,'<button class="btn btn-mini btn-danger"><i class="icon-trash bigger-120"></i></button>',array('onclick'=>"return confirm('Anda yakin akan menghapus data ini?')"))
                .
                '<div>'
            );
        }
    ...
    {
        $this->db->where('id_ksp', $id)->delete($this->db_tabel);
